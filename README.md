# Math Marauders — Arcade Math Sprint (Prototype)

This repository now contains a playable, mobile-friendly prototype of **Math Marauders**, capturing the forward-run gate picking, deterministic skirmish, reverse chase, and star-ranked end card loops described below. The experience runs entirely as a static web app with no build step—open `index.html` in any modern browser or run `npm run serve:static` for a local server. The HUD delivers compact score/time readouts, two-gate choice cards per decision, a log of combat beats, auto performance downgrades, and a pause drawer with restart/mute/FX toggles.
...
npm install
npm test
npm run serve:static # optional: host the static build at http://localhost:4173
```

### Prototype Highlights

- **Forward Run:** Three seeded, two-choice math gates per wave with rounded/clamped outcomes that respect the spec's clamp rules.
- **Skirmish:** Deterministic volleys every ~150 ms (simulated) using the damage curve from the spec; logs expose volley counts and survivors.
- **Reverse Chase:** Auto volleys drain the squad while a chasing horde surges at each cleared gate; success/failure feeds the end card.
- **HUD & UX:** Score/time/unit chips, delta flashes, steering slider, pause drawer (resume/restart/mute/FX), and run seed/chase status readouts.
- **Performance Guard:** Lightweight FPS monitor auto-switches to a low-effects mode when average FPS dips below the 50 FPS target.

The sections that follow preserve the original high-fidelity design brief for future iterations.
