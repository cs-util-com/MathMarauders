...
This repository now ships a playable three.js math-runner that implements the
core decision loop and renders the full 3D lane, units, and effects described
later in this spec. The current build delivers:

- **Forward math run:** deterministic gate generation seeded from the URL,
  rendered as in-world gate meshes with emissive numerals plus HUD telemetry for
  risk/yield decisions.
- **Skirmish resolution:** deterministic combat math that drives instanced unit
  volleys, projectile trails, and HUD deltas so the visual battle mirrors the
  logic layer.
- **Reverse chase & scoring:** a three.js chase sequence that feeds the end-card
  star calculation, compact score formatting, and instant restart behaviour.
- **HUD + pause suite:** mobile-first layout, steering slider, pause overlay
  with mute/low-mode toggles, and automatic performance guards wired to the live
  renderer.

### 3D implementation requirements (active scope)
